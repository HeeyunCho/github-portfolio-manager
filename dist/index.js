...
server.tool("sync_all_projects", "Scans the workspace and prepares git synchronization for all active projects.", {}, async () => {
    ...
    for (const dir of dirs) {
        ...
        if (!fs.existsSync(path.join(dirPath, ".git"))) {
            ...
            try {
                ...
                results.push(`Error initializing '${dir}': ${e.message}`);
            }
        }
        else {
...
server.tool("generate_profile_readme", "Generates a professional GitHub Profile README based on local project metadata.", {
    ...
    for (const dir of dirs) {
        if (dir.startsWith(".") || dir === "node_modules" || dir === "github-portfolio-manager")
            continue;
